...
define[define what class need to be decoded or encoded]
add[add code generation]
re[reflection: the default,and supprt private class or feild]
call[Call CodegenConfig setup before using encoder or decoder]
dy[dynamic code generation:require javassist library dependency,public class/field only]
static[static code generation:also an option]
define -. Only uesd when choosing dynamic code generation .-> dependency
define --> add
add --> call
call -.- re
call -.- static
...
![Any](https://github.com/glamormu/java/tree/master/img/Any.jpg)
